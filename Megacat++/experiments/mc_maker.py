...
#returns library key and vectorized dataset ==> {'key', 'dat_vec'}
def vectorize(dset):
...
def importMarioLevels(loc='../Sequential Level Design/mario_ai_levels/original/'):
    ...
    #open each level
    with tqdm(total=len(level_list)) as pbar:
...
def vecABCSongs():
    ...
    with open("../Music/abc_songs.txt") as f:
        ...
        #clean up songs
        with tqdm(total=len(full_songs)) as pbar:
	        for s in full_songs:
	            bare_song = []
	            for i in s.split("\n"):
...
def vecCellAuto():
    ...
    with open("../Cellular Automata/conway_samples_n6.txt") as f:
        ...
        with tqdm(total=len(cell_seq)) as pbar:
	        for seq in cell_seq:
	            gridSets = seq.split("\n\n")
	            ind_grids = []
    ...
    return {"data_vec": gridDat2Seq(gridSeqs), "key":gridSizes}
...
def multiChoice(dataset, minQuest = -1, seqRange=[5,15],ansRange=[3,6],lastProb=0.85):
    qdat = []
    ...
    #go over each item
    i = 0
    total = (len(shufDat) if minQuest == -1 else minQuest)
...
def testMaker(domain='arith',questions=100):
	print(f"-- Generating {questions} sequence questions from {domain} Domain --")
	vecset = {}
	if domain == "mario":
	...
	return {"library":vecset["key"].tolist(),"questions":multiChoice(vecset["dat_vec"],questions)}
